...
<body>
	...
	<div class="content">
		...
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-2">
					<a href='<c:url value="/admin/ourteam/create" />' id="payment-button"
									...
									<c:forEach var="item" items="${ourteams }">
										<tr>
											...
											<td><img alt="" height="50px"
												src="<c:url value="/assets/uploads/ourteams/${item.img }" />"
												onclick="openModal('<c:url value="/assets/uploads/ourteams/${item.img }" />')">
											</td>
											<td>${item.name }</td>
											<td>${item.nickname }</td>
											<td>${item.interactively }</td>
											<td><a
												href='<c:url value="/admin/ourteam/edit/${item.id }" />'
												type="button"><i
													class="fa fa-pencil-square text-success" aria-hidden="true"
													style="font-size: 22px;"></i></a></td>
											<td><form:form method="DELETE"
													action='ourteam/delete/${item.id}'>
