...
<body>
	...
	<div class="content">
		...
		<div class="animated fadeIn">
			<div class="row">
				<div class="col-lg-2">
					<a href='<c:url value="/admin/about/create" />' id="payment-button"
									...
									<c:forEach var="item" items="${abouts }">
										<tr>
											...
											<td><img alt="" height="50px"
												src="<c:url value="/assets/uploads/abouts/${item.img }" />"
												onclick="openModal('<c:url value="/assets/uploads/abouts/${item.img }" />')">
											</td>
											<td>${item.title }</td>
											<td>${item.description }</td>
											<td><a
												href='<c:url value="/admin/about/edit/${item.id }" />'
												type="button"><i
													class="fa fa-pencil-square text-success" aria-hidden="true"
													style="font-size: 22px;"></i></a></td>
											<td><form:form method="DELETE"
													action='about/delete/${item.id}'>
