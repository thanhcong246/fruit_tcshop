					...
					<div class="pagination-wrap">
						<ul>
							<c:if test="${paginateInfo.currentPage > 1}">
								<li><a
									href="<c:url value='/san-pham/category/${idCategory}/${paginateInfo.currentPage - 1}' />">Quay
										lại</a></li>
							</c:if>
							<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}"
								...
								<c:if test="${loop.index != paginateInfo.currentPage}">
									<li><a
										href="<c:url value='/san-pham/category/${idCategory}/${loop.index}' />"><c:out
												value="${loop.index}" /></a></li>
								</c:if>
							</c:forEach>
							<c:if test="${paginateInfo.currentPage < paginateInfo.totalPage}">
								<li><a
									href="<c:url value='/san-pham/category/${idCategory}/${paginateInfo.currentPage + 1}' />">Tiếp</a></li>
							</c:if>
						</ul>
					</div>
				</div>
